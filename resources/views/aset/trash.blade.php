<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-lg text-gray-800">Sampah Aset</h2>
        <p class="text-xs text-gray-500 mt-0.5">{{ $asetList->total() }} aset yang sudah dihapus, bisa dipulihkan</p>
    </x-slot>
    ...
    <div class="p-7 space-y-4 max-w-[1360px]">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <p class="text-sm text-gray-500 max-w-3xl">
                Aset yang dihapus ditaruh di sini dulu, gak langsung hilang permanen -- bisa dipulihkan kapan saja.
            </p>
            ...
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-cakrawala">
                    <tr>
                        ...
                        <th class="px-4 py-2.5 text-left text-[11px] font-bold text-white uppercase tracking-wide">SN</th>
                        <th class="px-4 py-2.5 text-left text-[11px] font-bold text-white uppercase tracking-wide">Dihapus Pada</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($asetList as $aset)
                        <tr class="hover:bg-cakrawala/5">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500">{{ $aset->deleted_at?->format('d M Y, H:i') }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                <form action="{{ route('aset.restore', $aset->id) }}" method="POST" class="inline" onsubmit="return confirm('Pulihkan aset ini?')">
                                    @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center">
                                ...
                                <p class="text-gray-400 text-sm">Sampah kosong -- gak ada aset yang dihapus.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
