        ...
        <div class="max-w-3xl mx-auto">
            ...
            <form
                ...
                x-data="{
                    ...
                    kodeAsetTerpilih: '{{ old('kode_aset_kode') }}',
                ...
                <x-card padding="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input type="text" name="pemegang_nama" value="{{ old('pemegang_nama') }}" x-bind:required="butuhPemegang" class="mt-1.5 block w-full" />
                            <x-input-error :messages="$errors->get('pemegang_nama')" class="mt-1.5" />
                        </div>
                        <div>
                            <x-input-label value="Jabatan" />
                            <x-text-input type="text" name="jabatan" value="{{ old('jabatan') }}" x-bind:required="butuhPemegang" class="mt-1.5 block w-full" />
                            <x-input-error :messages="$errors->get('jabatan')" class="mt-1.5" />
                        </div>
                        <div>
                            <x-input-label value="Personal Number (PN)" />
                            <x-text-input type="text" name="pemegang_pn" value="{{ old('pemegang_pn') }}" @change="cariUkerDariPn($event.target.value)" x-bind:required="butuhPemegang" class="mt-1.5 block w-full" />
                            <p class="text-xs mt-1.5" :class="statusLookupPn.startsWith('Ditemukan') ? 'text-green-600' : 'text-gray-400'" x-text="statusLookupPn"></p>
                            <p class="text-xs text-gray-400">Isi PN dulu biar Uker di atas otomatis terpilih (tetap bisa dipilih manual).</p>
                            <x-input-error :messages="$errors->get('pemegang_pn')" class="mt-1.5" />
                        </div>
                        <div>
                            <x-input-label value="IP Address" />
                            <x-text-input type="text" name="ip_address" value="{{ old('ip_address') }}" placeholder="contoh: 10.0.0.1" x-bind:required="butuhPemegang" class="mt-1.5 block w-full" />
                        ...
                        <div>
                            ...
                            <x-select name="status_hardening" x-bind:required="butuhPemegang" class="mt-1.5 block w-full">
                                ...
                                @foreach (\App\Models\Aset::DAFTAR_STATUS_SUDAH_BELUM as $s)
                                    <option value="{{ $s }}" @selected(old('status_hardening') == $s)>{{ $s }}</option>
                                @endforeach
                            </x-select>
                        ...
                        <div>
                            ...
                            <x-select name="status_bitlocker" x-bind:required="butuhPemegang" class="mt-1.5 block w-full">
                                ...
                                @foreach (\App\Models\Aset::DAFTAR_STATUS_AKTIF as $s)
                                    <option value="{{ $s }}" @selected(old('status_bitlocker') == $s)>{{ $s }}</option>
                                @endforeach
                            </x-select>
                        ...
                        <div>
                            ...
                            <x-select name="status_dlp" x-bind:required="butuhPemegang" class="mt-1.5 block w-full">
                                <option value="">-- Pilih Status --</option>
                                @foreach (\App\Models\Aset::DAFTAR_STATUS_AKTIF as $s)
                                    <option value="{{ $s }}" @selected(old('status_dlp') == $s)>{{ $s }}</option>
                                @endforeach
                            </x-select>
                        ...
                        <div>
                            ...
                            <x-select name="status_antivirus" x-bind:required="butuhPemegang" class="mt-1.5 block w-full">
                                <option value="">-- Pilih Status --</option>
                                @foreach (\App\Models\Aset::DAFTAR_STATUS_AKTIF as $s)
                                    <option value="{{ $s }}" @selected(old('status_antivirus') == $s)>{{ $s }}</option>
                                @endforeach
                            </x-select>
